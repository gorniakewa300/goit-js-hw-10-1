...
function createMarkUp(event) {
  ...
  fetchCatByBreed(breedId)
    .then(data => {
      ...
      textMarkEl.innerHTML = `<img src="${url}" alt="${breeds[0].name}" width="500"/><div class="box"><h2>${breeds[0].name}</h2><p>${breeds[0].description}</p><p><strong>Temperament:</strong> ${breeds[0].temperament}</p></div>`;
